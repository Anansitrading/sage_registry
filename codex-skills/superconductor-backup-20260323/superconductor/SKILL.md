...
name: superconductor
description: >-
  Autonomous spec-to-verified execution engine. Takes any requirements document
  and drives a brownfield or greenfield codebase to full implementation using the
  conductor workflow. Provides a single CLI (`superconductor`) that connects to
  codex app-server via WebSocket, sends focused per-wave assignments, monitors
  turns via turn/steer, runs a hard completion gate, and loops until ALL
  requirements are verified. Use when the user asks to run a full pipeline
  against a spec, execute superconductor, or autonomously implement a
  requirements document. Requires the conductor skill to be installed for
  project context (product.md, tech-stack.md, workflow.md, tracks).
metadata:
  author: DavidSkills
  version: '2.0'
---
...
ln -sf ~/.codex/skills/superconductor/bin/superconductor.mjs /usr/local/bin/superconductor

# Or use directly
node ~/.codex/skills/superconductor/bin/superconductor.mjs <subcommand> ...
```

Requires: Node.js 18+, `ws` package (`npm i -g ws`).
